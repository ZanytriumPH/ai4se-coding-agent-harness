...
- **WebUI 公网部署（§5.9）**：**公网 URL**：`https://<service-name>.onrender.com`（部署后在此填入实际 URL）。Render free 层，空闲 15min 后睡眠，冷启动 ~30s。架构：`uvicorn` 跑 Starlette `webui.server:app`，前端 Material Components Web CSS（CDN 引入），SSE `/events` 流式推送步骤、`GET /pending` 查待审批、`POST /approve` 回传审批。
  - **部署形态**：容器镜像（`Dockerfile`，默认 CMD `uvicorn webui.server:app`，读 `${PORT:-8000}`）。部署于 **Render**（`render.yaml` Blueprint）：连 GitHub 仓库 → 读 Dockerfile 构建 → web service 注入 `PORT` → 公网 `*.onrender.com`。**不烘焙 LLM key**（§3.1）：公网仅服务前端 + SSE/审批端点；真实带 key 的驱动跑留本地 `harness --run-webui`，避免公网暴露凭据。
  - **镜像到 GitHub**（Render 仅连 GitHub / GitLab.com，不认自建 NJU GitLab；同时落实 §4.7「公开 GitHub 仓库」）：在 github.com 建公开仓库 → `git remote add github https://github.com/<user>/<repo>.git` → `git push github main`。镜像前已全历史凭据扫描（无 `sk-`/api_key 明文，`.env` 从未进历史）。
  - **已知限制**：free 层睡眠 + 冷启动；公网 URL 仅前端（无驱动循环，log 初始为空）——这是 §5.9「可访问 WebUI 接口」的诚实形态，真实修复仍走本地配 key。
