...
```bash
make test     # pytest -q（76 个离线单测，全程无网络 / 无真实 LLM）
...
- **WebUI 公网部署（§5.9）**：**公网 URL**：`https://coding-agent-harness-production.up.railway.app/`。架构：Starlette `webui.server:app` + uvicorn，前端 Material Components Web CSS（CDN 引入），SSE `/events` 流式推送步骤、`GET /pending` 查待审批、`POST /approve` 回传审批。
  - **部署形态**：容器镜像（`Dockerfile`），部署于 **Railway**（连 GitHub 仓库 → 读 Dockerfile 构建 → web service 注入 `PORT`）。默认 CMD 驱动 **mock 修复循环**（`python -m harness.cli --run-webui --mock`，读 `${PORT:-8000}`）：公网 URL 上跑的是免 token 的确定性演示——`run_tests`（fail）→ `write_file`（绝对路径触发 guardrail → 浏览器弹出审批卡 → 访客点 Approve）→ `run_tests`（pass）→ 30s 后重启复演。**不烘焙 LLM key**（§3.1）：公网仅跑 mock，真实带 key 的驱动跑留本地 `harness --run-webui`，避免公网暴露凭据。
  - **镜像到 GitHub**（Railway 仅连 GitHub / GitLab.com，不认自建 NJU GitLab；同时落实 §4.7「公开 GitHub 仓库」）：在 github.com 建公开仓库 → `git remote add github https://github.com/<user>/<repo>.git` → `git push github main`。镜像前已全历史凭据扫描（无 `sk-`/api_key 明文，`.env` 从未进历史）。
  - **已知限制**：mock 循环在 turn 2 的审批卡处阻塞，等访客点 **Approve** 后才推进到 turn 3 完成（HITL 设计，确定性可复演——每次重启重放 `run_tests(fail)` → 审批 → `run_tests(pass)` → RUN RESULT，实测公网 URL 可见三幕全跑通）。不烘焙 LLM key（§3.1），真实 LLM 修复走本地 `harness --run-webui` 配 key。
